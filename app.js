...
// Final invoice export handler: opens a print-ready invoice matching the attached format.
window.exportInvoice = function exportInvoice(saleId){
  const sale = (typeof saleById === 'function' ? saleById(saleId) : null) || (state.sales || []).find(item => item.id === saleId);
  if (!sale) return alert('Invoice not found.');
  const amount = typeof totalFor === 'function' ? totalFor(sale) : Number(sale.total || sale.monthly || 0);
  const subtotal = sale.sst ? amount / 1.08 : amount;
  const sst = sale.sst ? amount - subtotal : 0;
  const moneyText = value => 'RM' + Number(value || 0).toLocaleString('en-MY', { minimumFractionDigits: 2, maximumFractionDigits: 2 });
  const clean = value => String(value || '').replace(/[&<>"']/g, ch => ({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[ch]));
  const longDate = value => new Date(value + 'T12:00:00').toLocaleDateString('en-MY', { day:'numeric', month:'long', year:'numeric' });
  const invoiceNumber = sale.invoiceNumber || 'Invoice';
  const win = window.open('', '_blank');
  if (!win) return alert('Please allow pop-ups to export invoice.');
  win.document.write(`<!doctype html><html><head><title>Invoice ${clean(invoiceNumber)}</title><style>
    @page{size:Letter;margin:0}*{box-sizing:border-box}body{margin:0;background:#eef1f5;font-family:Arial,Helvetica,sans-serif;color:#1f2933}.page{width:8.5in;min-height:11in;margin:0 auto;background:white;padding:.55in .62in;position:relative}.top{display:flex;justify-content:space-between;gap:40px}.brand h1{margin:0;font-size:24px;letter-spacing:.08em}.brand p{margin:6px 0 0;color:#697887;font-size:12px}.invoice{text-align:right}.invoice h2{margin:0 0 12px;font-size:38px;font-weight:500;letter-spacing:.08em}.seller{font-size:12px;line-height:1.55;color:#4b5563}.rule{height:1px;background:#d8dde3;margin:32px 0}.meta{display:grid;grid-template-columns:1fr 1fr;gap:32px}.label{font-size:11px;font-weight:700;color:#596779;text-transform:uppercase;letter-spacing:.08em}.bill strong{display:block;margin-top:10px;font-size:16px}.facts{font-size:13px}.fact{display:grid;grid-template-columns:145px 1fr;margin:8px 0}.amount-due{margin-top:16px;padding:12px 14px;background:#f3f5f7;border-radius:4px;display:flex;justify-content:space-between;font-weight:700}.items{width:100%;border-collapse:collapse;margin-top:26px;font-size:13px}.items th{background:#6d747d;color:white;text-align:left;padding:12px}.items td{border-bottom:1px solid #e3e7eb;padding:14px 12px;vertical-align:top}.items th:last-child,.items td:last-child{text-align:right}.totals{width:280px;margin:24px 0 0 auto;font-size:13px}.totals div{display:flex;justify-content:space-between;padding:8px 0;border-bottom:1px solid #e6eaee}.totals .grand{font-size:18px;font-weight:800;border-bottom:2px solid #111}.terms{margin-top:34px;font-size:12px;line-height:1.7;color:#4d5865}.footer{position:absolute;left:.62in;right:.62in;bottom:.35in;text-align:center;color:#87919c;font-size:11px}.actions{position:fixed;right:18px;top:18px;display:flex;gap:8px}.actions button{border:0;border-radius:999px;padding:10px 14px;font-weight:700;cursor:pointer}.primary{background:#1277ff;color:white}@media print{body{background:white}.page{margin:0}.actions{display:none}}
  </style></head><body><div class="actions"><button class="primary" onclick="window.print()">Save / Print PDF</button><button onclick="window.close()">Close</button></div><section class="page"><div class="top"><div class="brand"><h1>BRANDMARK</h1><p>insert your tagline here</p></div><div class="invoice"><h2>INVOICE</h2><div class="seller"><strong>Brandmark Sdn. Bhd.</strong><br>Malaysia<br>hello@brandmark.com</div></div></div><div class="rule"></div><div class="meta"><div class="bill"><div class="label">Bill To</div><strong>${clean(sale.client)}</strong></div><div class="facts"><div class="fact"><b>Invoice Number:</b><span>${clean(invoiceNumber)}</span></div><div class="fact"><b>Invoice Date:</b><span>${longDate(sale.date)}</span></div><div class="fact"><b>Payment Due:</b><span>${longDate(sale.date)}</span></div><div class="amount-due"><span>Amount Due (MYR):</span><span>${moneyText(amount)}</span></div></div></div><table class="items"><thead><tr><th>Items</th><th>Quantity</th><th>Monthly</th><th>Amount</th></tr></thead><tbody><tr><td><strong>${clean(sale.product)}</strong><br>${sale.type === 'recurring' ? clean((sale.months || 1) + ' month recurring service') : 'Lump sum service'}</td><td>${sale.type === 'recurring' ? clean(sale.months || 1) : 1}</td><td>${sale.type === 'recurring' ? moneyText(sale.monthly || subtotal) : '-'}</td><td>${moneyText(subtotal)}</td></tr></tbody></table><div class="totals"><div><span>Subtotal:</span><strong>${moneyText(subtotal)}</strong></div><div><span>SST 8%:</span><strong>${moneyText(sst)}</strong></div><div class="grand"><span>Total:</span><strong>${moneyText(amount)}</strong></div><div><span>Amount Due:</span><strong>${moneyText(amount)}</strong></div></div><div class="terms"><div class="label">Terms</div><p>Please make payment by the due date. Thank you for your business.</p></div><div class="footer">Page 1 of 1 for Invoice #${clean(invoiceNumber)}</div></section></body></html>`);
  win.document.close();
  win.focus();
};
